...
def generate_report(character: Character) -> str:
    ...
    # NPC对话体验
    npc_actions = [a for a in ["和镇子里的人打招呼", "向酒馆老板打听最近有没有什么有趣的事", "我和酒馆里的冒险者交谈", "询问关于附近怪物的情况"] 
                   if a in [n["action"] for n in all_narratives]]
    npc_narratives = [n for n in all_narratives if n["action"] in npc_actions]
...
#### NPC对话
- 探索中与 NPC 交互的次数：{len(npc_narratives)}
- NPC 回应是否具体：待观察
