...
class NarrativeCollector:
    ...
    async def narrative_handler(self, event: Event):
        try:
            ...
            if text:
                ...
                # 如果有等待这个回合的resolve
                if self._pending_turn == turn and self._pending_resolve:
    ...
    def wait_for_turn(self, turn_num, timeout=15):
        """等待特定回合的叙事"""
        self._pending_turn = turn_num
        loop = asyncio.get_event_loop()
        self._pending_resolve = asyncio.Event()
        
        async def wait_task():
            try:
                await asyncio.wait_for(self._pending_resolve.wait(), timeout=timeout)
                return True
...
class ExperienceGM:
    """封装 GameMaster 的体验官接口"""
    def __init__(self, master):
        self.master = master
        self.turn = 0
        
    async def handle(self, message, timeout=20):
        """发送输入并等待响应"""
        self.turn += 1
        ...
        handle_task = asyncio.create_task(
            self._safe_handle(message, current_turn)
        )
        
        # 等待叙事或超时
        wait_result = await asyncio.wait_for(
            self.master.event_bus._wait_for_event(
                EventType.NARRATIVE_OUTPUT,
                lambda e: e.data.get("turn") == current_turn,
                timeout=timeout
            ),
            timeout=timeout + 5
        )
    ...
    async def _safe_handle(self, message, turn_num, retries=0):
        """带重试的handle"""
        try:
            await self.master.handle_player_message(message)
        except Exception as e:
            err_str = str(e)
            is_rate = any(kw in err_str.lower() for kw in ["429", "rate", "too many", "timeout", "timed out", "network", "529", "overloaded", "500"])
            if is_rate and retries < RETRY_MAX:
                print(f"[API 限流，等待 30s 重试（第 {retries+1}/{RETRY_MAX} 次）]")
                await asyncio.sleep(RETRY_DELAY)
                return await self._safe_handle(message, turn_num, retries + 1)
            # 非限流错误，不重试但记录
            print(f"[Handle错误 #{turn_num}] {err_str[:100]}")
...
async def run_experience():
    ...
    for i, (tag, action) in enumerate(test_cases):
        ...
        print(f"输入: {action}")
        try:
            await exp_gm.handle(action, timeout=20)
        except asyncio.TimeoutError:
            print(f"[回合{i+1}超时]")
        except Exception as e:
            print(f"[回合{i+1}异常] {e}")
    ...
    # 统计
    empty_count = sum(1 for n in collector.narratives if len(n["text"].strip()) < 10)
    ...
    # 实际响应率
    responded = len([n for n in collector.narratives if len(n["text"].strip()) >= 10])
...
def generate_report(data) -> str:
    ...
    npc_turns = []
    for i, (tag, action) in enumerate(test_cases):
        if any(kw in tag for kw in ["NPC", "酒保", "冒险者", "商人", "铁匠", "深入"]):
            idx = i + 1
...
- **到达阶段**：{final_location}
- **死亡次数**：0
- **NPC对话次数**：{sum(1 for t in test_cases if any(kw in t[0] for kw in ['NPC', '酒保', '冒险者', '商人', '深入']))}
